...
/**
 * fetch_history - Write a function that fetches the history file log.
...
int write_hist(var_s *vars)
{
	...
	{
		_postsfl(node->str, fl);
		_postfl('\n', fl);
	}
	_postfl(BUF_FLUSH, fl);
	close(fl);
	return (1);
}
...
int read_hist(var_s *vars)
{
	...
	for (x = 0; x < fsize; x++)
		...
		{
			buf[x] = 0;
			build_hlist(vars, buf + last, l_count++);
			last = x + 1;
		}
	if (last != x)
		build_hlist(vars, buf + last, l_count++);
	free(buf);
	vars->hist_count = l_count;
...
/**
 * num_history - A function to renumbers the history linked list after changes.
 * @vars: Structure containing potential arguments. Used to maintain
 *
